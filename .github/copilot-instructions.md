...
This document provides guidance for GitHub Copilot when working with NOMAD atomistic (classical force-field / molecular-dynamics) parsers. Each parser extracts computational results from simulation output files and maps them to NOMAD's unified runschema.
...
```
atomisticparsers/{parser_name}/MAPPING_REPORT.md
```
...
{ echo "# Electronic parser mapping report"; echo;
  for f in $(ls atomisticparsers/*/MAPPING_REPORT.md | sort); do
...
- **Heading** (`## <Code> / <FILE>`): the code name and the output file the class parses, e.g.
  `## LAMMPS / log.lammps`, `## GROMACS / md.log`. One heading per file-parser class; a parser
  that reads several files (e.g. a text log plus a separate input/data file) gets one section
  each, all in the same `MAPPING_REPORT.md`. Note: many atomistic parsers read trajectories via
  MDAnalysis/ASE/HDF5 rather than declarative `Quantity` objects — that portion is not reportable
  here and, if a class has no `Quantity` list at all, use the Skipped-parser form below.
...
- **Archive mapper source**: for a `Mapped` row, the runschema attribute path it feeds (e.g.
  `energy.total`, `forces.total`, `atoms.positions`, `calculation.temperature`,
  `calculation.pressure`) or the helper that consumes it; for an `Unmapped` row, an em dash `—`.
  Join multiple targets with `<br>`. Wrap paths in backticks. These parsers are workflow-heavy, so
  many `Mapped` targets are `simulationworkflowschema.*` (e.g. `MolecularDynamics.method`,
  `ThermostatParameters.*`, `BarostatParameters.*`, `GeometryOptimizationMethod.*`) rather than
  `runschema.calculation.*` — cite whichever normalized schema the quantity actually reaches.
...
1. **Every** left-column quantity name MUST correspond to a real `Quantity(...)` in the parser's
   ...
   `grep -nE "Quantity\(" atomisticparsers/<code>/*.py`.
2. **Every** `Mapped` target MUST be a real attribute path in the installed `runschema` or
   `simulationworkflowschema` packages (cross-check against the installed sources, e.g.
   `python -c "import runschema, os; print(os.path.dirname(runschema.__file__))"`).
3. Do not omit quantities to inflate coverage: list every declared quantity, mapped or not —
   **including quantities whose `name` is built dynamically** (in a loop or from a dict/mapping,
   e.g. per-column thermo outputs or `x_<code>_*` keys built from a mapping). Resolve the runtime
...
1. Locate the declarative file-parser classes in the parser:
   `grep -nE "class .*\((TextParser|XMLParser)\)" atomisticparsers/<code>/parser.py`.
...
- Main runschema: the installed `runschema` package (`run.py`, `method.py`, `system.py`,
  `calculation.py`) — locate with
  `python -c "import runschema, os; print(os.path.dirname(runschema.__file__))"`.
- Workflow schema: the installed `simulationworkflowschema` package — locate with
  `python -c "import simulationworkflowschema, os; print(os.path.dirname(simulationworkflowschema.__file__))"`.
