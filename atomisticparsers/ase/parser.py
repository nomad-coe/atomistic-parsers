    ...
    def traj(self):
        if self._file_handler is None:
            try:
                ...
                if 'calculator' not in self._file_handler.backend.keys():
                    self.logger.error('No calculator specified in trajectory file.')
            except Exception:
...
class AseParser(MDParser): #change inheritance?, does not have to be molecular dynamics, can be simple geometry optimization
    def __init__(self):
    ...
    def parse_method(self):
        ...
        if traj[0].calc is not None:
            sec_method.force_field = ForceField(model=[Model(name=traj[0].calc.name)]) #does not have to be force field, could be DFT code as well, or machine learning potential
        ...
        if calc_type == 'optimization':
            ...
            workflow.x_asap_maxstep = description.get('maxstep', 0) #is GeometryOptimzation asap specific? rename to x_ase_maxstep?
    ...
    def write_to_archive(self) -> None:
        ...
        sec_run.program = Program(name='ASE', version=self.traj_parser.get_version())
