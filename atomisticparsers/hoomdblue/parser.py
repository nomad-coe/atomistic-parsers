...
#   'hpmc/simple_polygon/vertices']}
    ...
    # def parse(self, key):
    #     if self.filegsd is None:
    #         return

    #     val = self.filegsd.get(key, None)
    #     if self._results is None:
    #         self._results = dict()

    #     if val is not None:
    #         val = np.asarray(val)
    #     if key in self._gsdhoomd_keys:
    #         val = val / MOL * ureg.kJ
...
class HoomdblueParser:
    ...
    def parse_thermodynamic_data(self, frame):
    ...
    def parse(self, filepath, archive, logger):
        ...
        #     name='HOOMDBLUE', version=str(header.get('version', 'unknown')).lstrip('VERSION '))
